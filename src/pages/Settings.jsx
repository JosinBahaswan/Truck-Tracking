...
const Settings = () => {
  ...
  const fetchProfile = useCallback(async () => {
    try {
      ...
      console.log('👤 User ID in token:', tokenPayload?.userId || tokenPayload?.id || tokenPayload?.sub);
      ...
      const normalizedData = {
        ...
        lastName: rawData.lastName || rawData.last_name || rawData.name?.split(' ').slice(1).join(' ') || '',
        email: rawData.email || '',
        role: rawData.role || '',
  ...
  const handleAvatarUpload = async (e) => {
    ...
    if (file.size > 5 * 1024 * 1024) {
      setAlertModal({ isOpen: true, type: 'error', title: 'File Too Large', message: 'File size cannot exceed 5MB' });
      return;
    }
  ...
  const handleToggleTwoFactor = async (enabled) => {
    try {
      ...
      setAlertModal({ isOpen: true, type: 'success', title: 'Success!', message: `Two-factor authentication ${enabled ? 'enabled' : 'disabled'}` });
    } catch (error) {
      console.error('Failed to toggle two-factor:', error);
      setAlertModal({ isOpen: true, type: 'error', title: 'Failed', message: error.response?.data?.message || 'Failed to toggle two-factor' });
    }
  };
  ...
  const handleCreateUser = async (e) => {
    ...
    try {
      ...
      setAlertModal({ isOpen: true, type: 'success', title: 'Success!', message: 'User created successfully!' });
      setNewUserForm({
        name: '',
        email: '',
        password: '',
  ...
  const handleChangePassword = async (e) => {
    ...
    if (!passwordForm.currentPassword) {
      setAlertModal({ isOpen: true, type: 'error', title: 'Error', message: 'Current password is required' });
      return;
    }
    
    if (passwordForm.newPassword.length < 6) {
      setAlertModal({ isOpen: true, type: 'error', title: 'Invalid Password', message: 'New password must be at least 6 characters' });
      return;
    }
    
    if (passwordForm.newPassword !== passwordForm.confirmPassword) {
      setAlertModal({ isOpen: true, type: 'error', title: 'Password Mismatch', message: 'Passwords do not match' });
      return;
    }
    ...
    try {
      ...
      setAlertModal({ isOpen: true, type: 'error', title: 'Failed', message: error.response?.data?.message || 'Failed to change password' });
    }
  };
...
{JSON.stringify(profile, null, 2)}
                ...
                <div className="mt-6 border-t border-gray-100 pt-6">
                  ...
                  <div className="space-y-2">
                    {['Full System Access', 'Manage Users', 'Fleet Configuration'].map((permission) => (
                ...
                {profile?.avatar && (
                  <div className="mt-6 border-t border-gray-100 pt-6">
                    <button onClick={handleDeleteAvatar} className="w-full px-3 py-2 bg-red-50 text-red-600 rounded-lg text-sm hover:bg-red-100 border border-red-200">Delete Avatar</button>
                  </div>
                )}
              </div>
            </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {profile?.username && (
                      <div className="md:col-span-2">
                        ...
                        <p className="text-sm text-gray-900 bg-gray-50 px-3 py-2.5 rounded-lg border border-gray-200">{profile.username}</p>
                      </div>
                    )}
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">
                      ...
                      <p className="text-sm text-gray-900 bg-gray-50 px-3 py-2.5 rounded-lg border border-gray-200">{profile?.firstName || '-'}</p>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">
                        Last Name
                      </label>
                      <p className="text-sm text-gray-900 bg-gray-50 px-3 py-2.5 rounded-lg border border-gray-200">{profile?.lastName || '-'}</p>
                    </div>
                    <div className="md:col-span-2">
                      <label className="block text-sm font-medium text-gray-700 mb-2">
                        Email Address
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-900 bg-gray-50 px-3 py-2.5 rounded-lg border border-gray-200">
                        {profile?.department || <span className="text-gray-400 italic">Not assigned</span>}
                      </p>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">
                    ...
                    <div>
                      ...
                      <p className={`text-sm font-medium px-3 py-2.5 rounded-lg border capitalize ${
                        profile?.status === 'active' 
                    ...
                    <div className="md:col-span-2">
                      ...
                      <p className="text-sm text-gray-900 bg-gray-50 px-3 py-2.5 rounded-lg border border-gray-200 min-h-[80px]">{profile?.bio || 'No bio provided'}</p>
                    </div>
                    {/* <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">
